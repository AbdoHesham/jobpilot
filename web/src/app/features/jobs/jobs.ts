...
const TABS: ReadonlyArray<{ value: JobStatus; label: string }> = [
  { value: 'new', label: 'New' },
    ...
    @if (jobService.isLoading()) {
      ...
      <div class="card empty">
        @if (!hasSearches()) {
          ...
          <p class="muted">
            Create a <a routerLink="/searches">search profile</a> first — that's what the feed is
            built from.
          </p>
        } @else {
          <h2>Nothing here</h2>
          <p class="muted">Hit Refresh to pull the latest postings for your active searches.</p>
        }
      </div>
    } @else {
      <ul class="list">
        @for (job of jobs(); track job.id) {
          <li class="card item">
            <div class="head">
              ...
              <div class="badges">
                @if (job.publisher) {
                  <span class="badge">{{ job.publisher }}</span>
                }
...
export class Jobs {
  ...
  private readonly searchProfiles = inject(SearchProfileService);
  ...
  protected readonly hasSearches = signal(true);
  ...
  private async load(): Promise<void> {
    try {
      await Promise.all([
        this.jobService.reload(this.status()),
        this.searchProfiles.reload(),
        this.cvService.reload(),
      ]);
      this.hasSearches.set(this.searchProfiles.profiles().length > 0);
    } catch (error) {
      this.fail(error);
    }
  }
  ...
  protected async select(status: JobStatus): Promise<void> {
    this.status.set(status);
    try {
      await this.jobService.reload(status);
    } catch (error) {
      this.fail(error);
    }
  }
  ...
  protected async refresh(): Promise<void> {
    ...
    try {
      ...
      await this.jobService.reload(this.status());
